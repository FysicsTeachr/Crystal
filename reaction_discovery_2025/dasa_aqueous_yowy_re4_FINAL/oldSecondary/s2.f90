...
foundling="foundling.txt"
            ...
            open(unit=14, file="test2/slurm-out")
            ...
            do
              ...
              if(index(text, " Final converged energy:").ne.0) then
               if(index(text,"-") .ne. 0) then
                first= index(text, "-")
                xxx= text(first:first+14)
                energyChecked=1
               end if
              end if
              ...
              if(index(text, "ChemShell exiting code 0").ne.0) then
                 energyChecked=1
                 exit
              end if
              ...
              if(index(text, "Energy evaluation failed").ne.0) then
                energyChecked=3
                exit
              end if
              if(index(text, "Cyclic failure at residue").ne.0) then
                energyChecked=3
                exit
              end if

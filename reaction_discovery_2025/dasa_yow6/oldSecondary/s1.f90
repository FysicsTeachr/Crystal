...
foundling="foundling.txt"
  ...
  read(110,*) pre, qq2d
  close(110)
!  write(pre_str, '(I0)') pre
!  pre_str=trim(adjustl(pre_str))
!  command= 'cp ../yesfiles/' // trim(pre_str) // '.txt pregeom.xyz' 
!  call execute_command_line(trim(adjustl(command)), exitstat=ios)
  
!  if (ios /= 0) then
!  open(unit=106, file='../danger.txt', status='unknown', action='write', position='append')
!  write(106, '(A)') trim(pre_str)
!  close(106)
!  end if

  open(110,file='../prexyz.txt')
  open(106,file='pregeom.xyz')
  do i=1,current-1
    do j=1,44
      read(110,*)
    end do
  ...
  write(106,*)
  do i=1,42
    read(110,*) reader, x, y, z
    write(106,*) reader, x, y, z
  end do
  close(110)
  close(106)

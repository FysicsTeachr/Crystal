...
bbb=400   !number of secondary jobs

allocate(yesfiles(100000,79,3))
allocate(nofiles(1000000,79,3))
allocate(nofilesTMP(1000000,79,3))
...
dump=0
Ecut0= -272.425   !!! cutoff energy for accepting lattice
incr=0.055
EcutMax=-272.365
...
!      end do
  ...
  ! 132: if vbarTMP<=Ecut0 then transfer file i-1.xyz from nofolder to yesfolder and rename it as ibar.xyz
  !Also copy it to climbing lattices

      write(command, '("mv nofolder/", i0, ".xyz yesfolder/", i0, ".xyz")') i-1, ibar
      call execute_command_line(command)

      write(command, '("cp yesfolder/", i0, ".xyz climbinglat/")') ibar
      call execute_command_line(command)
 ...
 do i=ci,cf
  ...
  do t=1,d
   do tsign = -1,1,2
    ...
    if(qq2(1).le.3 .and. qq2(2).le.3 .and. qq2(3).le.3 .and. qq2(4).le.3  &
    .and. qq2(1)>-3 .and. qq2(2)>-3 .and. qq2(3)>-3 .and. qq2(4)>-1 &
    .and. qq2(5).le.3 .and. qq2(6).le.3 .and. qq2(7).le.0 .and. qq2(8).le.0  &
    .and. qq2(5)>-3 .and. qq2(6)>-3 .and. qq2(7)>-1 .and. qq2(8)>-1 &
    .and. Abs(qq2(9))<1 .and. Abs(qq2(10))<1 .and. Abs(qq2(11))<1  &
    .and. Abs(qq2(12))<1 .and. Abs(qq2(13))<1 .and. Abs(qq2(14))<1  )then
...
if(ibar3>1) then
 ...
 do i=2,ibar3
   ...
   !copy mia(i).xyz from yesfolder to prefolder
   ...
   write(command, '("cp yesfolder/", i0, ".xyz prefolder/")') mia(i)
   call execute_command_line(command)
...
! Additional debug statements to track file position
   ...
   if(vbarTMP.le.Ecut0) then
     ...
     if(unique.ne.0) then
       ...
       !transfer numbr.xyz from allfolder to yesfolder and rename it as ibar.xyz
       write(command, '("mv allfolder/", i0, ".xyz yesfolder/", i0, ".xyz")') numbr, ibar
       call execute_command_line(command)
...
!       end if
      ...
      write(command, '("cp yesfolder/", i0, ".xyz newlat/")') ibar
      call execute_command_line(command)
...
!       end if
       ...
       write(command, '("mv -f allfolder/", i0, ".xyz nofolder/", i0, ".xyz")') numbr, ibar2-1
       call execute_command_line(command)

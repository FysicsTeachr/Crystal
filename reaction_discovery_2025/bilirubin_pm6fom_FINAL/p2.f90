 ...
 do i=ci,cf
  ...
  do t=1,d
   do tsign = -1,1,2
    ...
    if(qq2(2).le.4 .and. qq2(4).le.4 .and. qq2(6).le.4 .and. qq2(8).le.4 .and. qq2(10).le.4  &
    .and. qq2(2)>-4 .and. qq2(4)>-4 .and. qq2(6)>-4 .and. qq2(8)>-4 .and. qq2(10)>-4 .and. &
...
if(ibar3>1) then
 ...
 !open the folder prefolder and copy mia(i).pun from yesfolder to it.
write(command, '("cp yesfolder/", i0, ".pun prefolder/", i0, ".pun")') mia(i), ibar3-1
...
!write(*,*) 'read complete'
   ...
   !foundp has the numbr/xi variable. it is being looped above 
   !if vbartmp<Ecut0, then if numbr/xi exists in allfolder, then save it to yesfolder as ibar.pun
!   if (vbartmp < Ecut0) then
     ! Construct the system command
!     write(command, '("mv -f allfolder/", i0, ".pun yesfolder/", i0, ".pun")') numbr, ibar
...
!       end if
       ...
       end do          !should thos part not be inside the previous if bracket?

     end if
!write(*,*) 'uniq0 action complete <'

   !if vbartmp>Ecut0, then if numbr/xi exists in allfolder, then save it to nofolder as ibar2.pun
...
! write(*,*) "allfiles distributed"
 ...
 !Remove all files from allfolder
 command = "rm -f allfolder/*"
 call execute_command_line(command)
